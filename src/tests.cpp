...
TEST(Test, max_size_testing) {
  ...
  //        EXPECT_EQ(logger.char_Vector.max_size(),
  //        logger.origin_char.max_size());
...
TEST(queue_test_suit, copy_constructor_test) {
...
TEST(queue_test_suit, move_constructor_test) {
  std::queue<char> orig_q1;
  orig_q1.push('a');
  orig_q1.push('b');
  orig_q1.push('c');
...
TEST(queue_test_suit, move_operator_test) {
  std::queue<char> orig_q1;
  orig_q1.push('a');
  orig_q1.push('b');
  orig_q1.push('c');
...
TEST(queue_test_suit, emplace_test) {
  std::queue<char> orig_q1;
  orig_q1.push('a');
  orig_q1.push('b');
  orig_q1.push('c');
...
// MULTISET
 ...
 TEST(Multiset, function_max_siez_not_empty) {
     TestMultiset tester;
     EXPECT_EQ(tester.s21_multiset_ten.max_size(), tester.std_multiset_ten.max_size());
...
EXPECT_EQ(s21_float_empty.max_size(), std_float_empty.max_size());
    ...
    TEST(Multiset, function_merge_with_duplicates) {
        ...
        EXPECT_EQ(tester.std_multiset_merge.empty(), tester.std_multiset_merge.empty());
    }
    ...
    TEST(Multiset, function_clear_empty) {
        ...
        EXPECT_EQ(tester.s21_multiset_empty.size(), tester.std_multiset_empty.size());
        EXPECT_EQ(tester.std_multiset_empty.empty(), tester.std_multiset_empty.empty());
    }
...
TEST(MultisetLowerBoundFunction, LowerBound) {
    s21::multiset<int> s1({1, 3, 4, 2, 2});
    std::multiset<int> s2({1, 3, 4, 2, 2});
...
TEST(MultisetUpperBoundFunction, UpperBound) {
    s21::multiset<int> s1({1, 3, 4, 2, 2});
    std::multiset<int> s2({1, 3, 4, 2, 2});
...
TEST(MultisetEqualRangeFunction, EqualRange) {
    s21::multiset<int> s1({1, 3, 4, 2, 2});
    std::multiset<int> s2({1, 3, 4, 2, 2});
...
TEST(MapEmplaceFunction, Emplace) {
...
TEST(MapContainsFunction, Contains) {
    s21::map<int, int> s1({{1, 3}, {4, 2}});
